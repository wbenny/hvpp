#pragma once
#include "ia32/arch.h"
#include "ia32/vmx/exit_qualification.h"
...
// So in the moment when interception of 0x5658 and 0x5659 I/O ports is
// established, they won't actually reach the VM-exit I/O handler at all,
// because #GPs will be suddenly raised (no matter if you catching #GP via
// exception bitmap or not).
//
...
// instructions changing these registers). This must be solved with custom ASM
// function (see vmware.asm). Also, we must not forget to write back those
// mentioned registers back to the guest state (except the RFLAGS.DF), because
// VMWare uses them for both input and output.
//
// After that the current instruction MUST be skipped AND the #GP must NOT be
// reinjected. This is obvious - we want to continue execution after the in/out
// instruction like nothing unusual happened.
//
