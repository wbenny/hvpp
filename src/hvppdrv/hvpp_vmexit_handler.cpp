...
void hvpp_vmexit_handler::setup(vcpu_t& vp) noexcept
{
  base_type::setup(vp);
...
void hvpp_vmexit_handler::handle_execute_cpuid(vcpu_t& vp) noexcept
{
...
void hvpp_vmexit_handler::handle_execute_vmcall(vcpu_t& vp) noexcept
{
  auto& data = data_[mp::cpu_index()];
  static bool applied = false;
  switch (vp.exit_context().rcx)
  {
    case vmcall_apply_shadow_page:
      __debugbreak();
      if (!applied)
      for (auto&& page : shadow_page_)
      {
        //
        // Split the 2MB page where the code we want to hook resides.
        //
        vp.ept().split_2mb_to_4kb(page.execute & ept_pd_t::mask, page.execute & ept_pd_t::mask);

        //
        // Set execute-only access on the page we want to hook.
        //
        vp.ept().map_4kb(page.execute, page.execute, epte_t::access_type::execute);
      }
      applied = true;
      //
...
void hvpp_vmexit_handler::handle_ept_violation(vcpu_t& vp) noexcept
{
  ...
  if (exit_qualification.data_read || exit_qualification.data_write)
  {
    ...
    //
    TraceFormat("data_read LA: 0x%p PA: 0x%p", guest_la, guest_pa.value());

    for (auto&& page : shadow_page_)
    {
      if ((page.execute & ept_pt_t::mask) == (guest_pa & ept_pt_t::mask))
      {
        vp.ept().map_4kb(page.execute, page.read_write, epte_t::access_type::read_write);
        ppp = true;
        break;
      }
    }
  ...
  else if (exit_qualification.data_execute)
  {
    ...
    //
    TraceFormat("data_execute LA: 0x%p PA: 0x%p", guest_la, guest_pa.value());
